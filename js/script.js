...
const intro_transition_front = document.querySelector(
  ".intro-transition-front"
);
const intro_transition_behind = document.querySelector(
...
tombol_continue.addEventListener("click", () => {
  setDisplay(true);
  intro_transition_front.style.animation =
    "hai-front 3.5s cubic-bezier(.75,.14,.13,.86) forwards";
  intro_transition_behind.style.animation =
    "hai-behind 3.6s cubic-bezier(.75,.14,.13,.86) forwards";
...
intro_transition_front.addEventListener("animationstart", () => {
  ...
  setInterval(() => {
    if (waktu === 0)
      return (intro_container.style.transform = "translateY(-100%)");
